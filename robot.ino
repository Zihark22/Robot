...
unsigned char creeper[] = {00, 0x00, 0x00, 0x00, 0x00, 0x0c, 0x6c, 0x30, 0x30, 0x6c, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x00};
...
int LEDstate = 0;   // état LED
IRrecv irrecv(RECV_PIN);
decode_results results;

OLED  myOLED(SDA_OLED, SCL_OLED);
char *msg;
...
void loop() {
  ...
  switch (BLE_val)
  {
    ...
    case 'd' : 
      ...
      break;
    case  'U':  
      follow();  //Receiving ‘U’,enter follow mode
      break;
    case  'Y':  
      avoid(); //Receiving ‘Y’,enter obstacle avoidance mode
      break;
    case  'G':  
      confinement(); //Receiving ‘G’,enter confinement mode
      break;
    case  'X':  
      tracking(); //Receiving ‘X’,enter tracking mode
      break;
  }
  ...
  {
    if (results.value == 0xFF02FD && (LEDstate == 0)) //according to the above key value, press“OK”on remote control , LED will be controlled
    {
      Serial.println("HIGH");
    ...
    else if (results.value == 0xFF02FD && (LEDstate == 1)) //press again
    {
      Serial.println("LOW");
    ...
    else if (results.value == 0xFF629D) // haut
    {
      car_front();
      msg="IR: ^ - Avance";
    }
    else if (results.value == 0xFFA857) // bas
    {
      car_back();
      msg="IR: v - Recule";
    }
    else if (results.value == 0xFFC23D) //droite
    {
      car_right();
      msg="IR: > - Droite";
    }
    else if (results.value == 0xFF22DD) // gauche
    {
      car_left();
      msg="IR: < - Gauche";
    }
    ...
    {
      matrix_display(clear);
      matrix_display(creeper);
      msg="IR: * - Creeper";
    }
    else if (results.value == 0xFF4AB5) // 0
    {
      msg="IR: 0 - Bitmap";
      afficheOLED();
      ...
      // OLED
      myOLED.clrScr();
      myOLED.drawBitmap(0, 0, oeil, 128, 65);
      myOLED.update();
      delay(200);
      for (int i = 0; i < 2; i++)
      {
    ...
    else if (results.value == 0xFF6897) // 1
    {
      msg="IR: 1 - Msg createur";
      afficheOLED();
      delay(1000);
      int y = random(0, 56);
      msg="Bonjour, je suis le createur Julien.B";
      for (int i = 128; i >= -(34 * 6); i -= 5)
      {
        myOLED.clrScr();
        myOLED.print(msg, i, y);
        myOLED.update();
      }
      delay(500);
      msg="Affichage termine";
    }
    else if (results.value == 0xFF9867) // 2
    {
      msg="IR: 2";
    }
    else if (results.value == 0xFFB04F) // 3
    {
      msg="IR: 3";
    }
    else if (results.value == 0xFF30CF) // 4
    {
      msg="IR: 4 - Speed Down";
      matrix_display(clear);
    ...
    else if (results.value == 0xFF18E7) // 5
    {
      msg="IR: 5";
    }
    else if (results.value == 0xFF7A85) // 6
    {
      msg="IR: 6 - Speed Up";
      matrix_display(clear);
    ...
    else if (results.value == 0xFF10EF) // 7
    {
      msg="IR: 7";
    }
    else if (results.value == 0xFF38C7) // 8
    {
      msg="IR: 8";
    }
    else if (results.value == 0xFF5AA5) // 9
    {
      msg="IR: 9";
    }
    afficheOLED();
...
void avoid() {
  ...
  while (avoid_flag) {
    ...
    if (distance > 0 && distance < 20) { //If the distance is less than 20 and greater than 0
      ...
      matrix_display(front);   //the dot matrix displays a forward pattern
      Serial.println("LOW");
      digitalWrite(LED_PIN, LOW); //LED will be on
      LEDstate = 0;
    }
    BLE_val = Serial.read();
    if (BLE_val == 'S') {//When S is received, the car stops
